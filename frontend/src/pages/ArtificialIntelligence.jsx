        ...
        try {
            await Promise.all(selectedFiles.map(async (file) => {
                try {
                    ...
                    await axios.post(
                        `${import.meta.env.VITE_API_BASE_URL ?? ''}/api/ai/datasets/upload`,
                        formData,
                        {
    ...
    const handleLoadDatasets = async () => {
        ...
        try {
            const response = await axios.post(
                `${import.meta.env.VITE_API_BASE_URL ?? ''}/api/ai/datasets/load`,
                { datasetNames },
                {
                    params: {
                        tenantUrl: user.tenantUrl,
...
const QuestsTab = ({ createdAlgorithms = [] }) => {
    ...
    const executeDeployments = async (deployments) => {
        ...
        for (const deploy of deployments) {
            ...
            try {
                ...
                await axios.post(
                    `${import.meta.env.VITE_API_BASE_URL ?? ''}/api/ai/quests`,
                    formData,
                    {
                        headers: { 'Content-Type': 'multipart/form-data' },
                        params: {
    ...
    const handleTrain = async () => {
        ...
        try {
            const response = await axios.post(
                `${import.meta.env.VITE_API_BASE_URL ?? ''}/api/ai/quests/train`,
    ...
    const handleDeploy = async () => {
        ...
        for (const file of selectedFiles) {
            try {
                ...
                await axios.post(
                    `${import.meta.env.VITE_API_BASE_URL ?? ''}/api/ai/optimization/quests`,
                    formData,
                    {
                        headers: { 'Content-Type': 'multipart/form-data' },
                        params: {
...
const CustomAlgorithmTab = ({ setCreatedAlgorithms }) => {
    ...
    const handleDeploy = async () => {
        ...
        try {
            const response = await axios.post(
                `${import.meta.env.VITE_API_BASE_URL ?? ''}/api/ai/custom-algorithms/deploy`,
        ...
        try {
            ...
            for (const zipFile of codeFiles) {
                ...
                try {
                    ...
                    const metaResponse = await axios.post(`${import.meta.env.VITE_API_BASE_URL ?? ''}/api/ai/custom-algorithms`, metadataPayload, {
                        params: {
                            tenantUrl: user.tenantUrl,
                            token: user.token,
                    ...
                    await axios.post(`${import.meta.env.VITE_API_BASE_URL ?? ''}/api/ai/custom-algorithms/upload-code`, codeFormData, {
                        headers: { 'Content-Type': 'multipart/form-data' },
                        params: {
                            tenantUrl: user.tenantUrl,
                            token: user.token,
                    ...
                    if (hyperFile) {
                        ...
                        await axios.post(`${import.meta.env.VITE_API_BASE_URL ?? ''}/api/ai/custom-algorithms/upload-hyperparams`, hyperFormData, {
                            headers: { 'Content-Type': 'multipart/form-data' },
                            params: {
                                tenantUrl: user.tenantUrl,
                                token: user.token,
